...
void Init_IOs(void)
{
	...
	PORTECFG = 0;		// Port E is a standard GPIO

	// Initialize directions for each ports
	OEE = 0x03;	// Port E0 and E1 are outputs, all other are inputs

	// Initialize output Ports default values
	IOE = 0x08;	// z8_reset=0, ocd_en=0, force_ocd_pin_to_zero = 1
}
...
void SyncToZ8Board(void)
{
	...
	// Turn on timer 0 (2 ms timer to detect timeout)
	TR0 = 1;
