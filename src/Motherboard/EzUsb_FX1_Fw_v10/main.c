...
//	sbit sync24_state_z_ez0_bit 			= sync24_state_z_ez0;
	...
	sbit midi_thru_ez0_bit 					= midi_thru_ez0;
...
void Check_Midi_In_Buffer(Midi_In_Struct *midi_struct, bit merger_src_enable)
{
	...
	// Check if something must be sent to Z8 Board (JLABS sysex or pure midi if arpeggio on)
...
void Check_Overflow_Flags(void)
{
	...
	{
		// send to Z8 board
		Send_Single_Framed_byte_To_Z8(LCD_MESSAGE_TO_Z8_INDEX, midi_in_realtime_overflow_z8);
		// clear overflow flag
	...
	{
		// send to Z8 board
		Send_Single_Framed_byte_To_Z8(LCD_MESSAGE_TO_Z8_INDEX, midi_in_overflow_z8);
		// clear overflow flag
	...
	if(usb_state_from_ez0_bit)
	{
		...
		{
			// send to Z8 board
			Send_Single_Framed_byte_To_Z8(LCD_MESSAGE_TO_Z8_INDEX, usb_in_realtime_overflow_z8);
			// clear overflow flag
		...
		{
			// send to Z8 board
			Send_Single_Framed_byte_To_Z8(LCD_MESSAGE_TO_Z8_INDEX, usb_in_overflow_z8);
			// clear overflow flag
...
void Check_Realtime(void)
{
	...
	Check_Realtime_Buffers(&MIDI_In_Realtime_From_Con, /*merger_src_midi_in_state_z_ez0_bit*/midi_thru_ez0_bit);
	...
	if(usb_state_from_ez0_bit)
	{
		...
		Check_Realtime_Buffers(&MIDI_In_Realtime_From_Usb, /*merger_src_usb_in_state_z_ez0_bit*/midi_thru_ez0_bit);
	}
}
