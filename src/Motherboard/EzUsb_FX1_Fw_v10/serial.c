...
idata unsigned char TX_To_Serial_Z8_Buff[SERIAL_TX_BUFF_LENGTH];
...
unsigned char code jlabs_sysex_header[] = {
MIDI_SOX, JLABS_SYX_ID0, JLABS_SYX_ID1, JLABS_SYX_ID2, JLABS_SYX_ID3, JLABS_SYX_ID4 
};
...
//				- MAWZER dedicated incoming sysex
//				- arpp is On and not a sysex has been received
// ***************************************************************************
...
//	EA = 1;
	...
	while(start_index_local != end_index_local)
	{
		...
		// Check it is a jlabs dedicated sysex (check if all jlabs sysex header is present)
		sysex_index = 0;	
		while((midi_struct->buffer[(start_index_local + sysex_index)&(MIDI_IN_FROM_CON_BUFF_LENGHT-1)] 
			  	== jlabs_sysex_header[sysex_index]) && (sysex_index < JLABS_SYX_HEADER_SIZE))
				sysex_index++;
	
		// a whole jlabs sysex header was in the buffer ?
		if(sysex_index == JLABS_SYX_HEADER_SIZE)
		{
			// also check this sysex is an incoming sysex for the 3X, as opposed to a outgoing sysex
			if(!(midi_struct->buffer[(start_index_local+JLABS_SYX_HEADER_SIZE)&(MIDI_IN_FROM_CON_BUFF_LENGHT-1)] & sysex_direction_sysxid0))
			{
				...
				if(midi_struct->buffer[(start_index_local+midi_mess_size-1)&(MIDI_IN_FROM_CON_BUFF_LENGHT-1)]	== MIDI_EOX)
				{
						...
						if(bootloader_state == BOOTLOADER_OFF)
						{
							...
							if( (midi_struct->buffer[(start_index_local+JLABS_SYX_HEADER_SIZE)&(MIDI_IN_FROM_CON_BUFF_LENGHT-1)] == (/*v1.1 FRAME_IS_FIRMWARE*/FRAME_IS_FIRMWARE_OR_PURE_MIDI | acknoledge_sysxid0 | checksum_check_sysxid0))
							  // data lenght = 127 is used to start bootloader
							  &&(midi_struct->buffer[(start_index_local+JLABS_SYX_HEADER_SIZE+1)&(MIDI_IN_FROM_CON_BUFF_LENGHT-1)] == DATA_LENGHT_START_BOOTLOADER) )
							  	// flag the bootloader to start it and exit
								must_start_bootloader_from_ez0_bit = TRUE;
							// send to z8 this incoming sysex
							else
							{
								must_send_sysex_to_z8 = TRUE; // it is a non bootloader jlabs sysex
...
//							USB_LED_PORT_OUT &= ~USB_LED_PIN;
			...
			} // end incoming jlabs sysex
		} // end it was a jlabs sysex (header fully recognized)
			
		
		// send all midi to z8 if arpeggio on and src corresponding enabled
		// if sysex_index is different than 0 then another NON-jlabs sysex has been received, do not forward it to z8
		// if it was a jlabs sysex ignore the following
		if(!must_send_sysex_to_z8)	// it was not a jlabs sysex
		{
...
//			else
				// it was a NON jlabs sysex : do not forward to z8 and return 
//				return;
		} // end it was not a jlabs sysex
		...
		if(must_send_to_z8)
		{
			...
			// a jlabs sysex has to be forwarded to z8; skip sysex header 
			if(must_send_sysex_to_z8)
			{
				// then next byte to send is the usefull bytes of received sysex skip (f0 00 20 4f 00 01 )
				start_index_local = (start_index_local + JLABS_SYX_HEADER_SIZE)&(MIDI_IN_FROM_CON_BUFF_LENGHT-1);
				// skip eox f7
				...
				midi_mess_size = midi_mess_size - JLABS_SYX_HEADER_SIZE - 1;
