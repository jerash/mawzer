...
void Check_If_Usb_Attached(void)
{
	...
	bit usb_monitor_input = VUSB_PORT_IN & VUSB_PIN;

	// usb was NOT present
	if(!usb_state_from_ez0_bit)
	{
		// is the vusb present ?
		if(usb_monitor_input)
		{
...
//		USB_LED_PORT_OUT |= USB_LED_PIN;
		...
		if(z8_board_fully_updated_from_ez0_bit)
		{
			...
			if(!bootloader_state)
				Send_Single_Framed_byte_To_Z8(CTRL_REG_0_7B_FROM_EZ_INDEX, CTRL_REGS_7BITS_TO_Z8[0]);
		}
...
//		return;
	...
	// check EP4 FULL(busy) bit in EP2468STAT (SFR), core set's this bit when FIFO is full
...
//	timeout_cpt = USB_LED_PERSISTENCY;
	...
	EP4FIFOBUF[0] = ((CN << 4) & 0xF0) | (USB_SINGLE_BYTE & 0x0F);		
	EP4FIFOBUF[1] = realtime_data;
	EP4FIFOBUF[2] = 0;
	EP4FIFOBUF[3] = 0;
    ...
    EP4BCH = 0;  
    SYNCDELAY;  
    EP4BCL = MIDI_USB_FRAME_LENGHT;  // arm EP4IN
    SYNCDELAY;                    
...
//		must_skip_header_frame = FALSE;
		...
		//if(!(EPIO[USB_IN_ID].cntrl & bmEPBUSY))
		{
			...
			{
				status = midi_struct->buffer[start_index_local];
			...
			for(i=0; i < MIDI_USB_FRAME_LENGHT; i++)
				...
				EP4FIFOBUF[i + usb_buffer_index] = usb_frame[i];				
...
//				if(!(EPIO[USB_IN_ID].cntrl & bmEPBUSY))
			    ...
			    if(!(EP2468STAT & bmEP4FULL))
				{
					...
					EP4BCH = 0;  
					SYNCDELAY;  
					EP4BCL = usb_buffer_index;  // arm EP4IN
					SYNCDELAY;                    
				}
				usb_buffer_index = 0;
	 		}
...
{ 
	unsigned char cin, frame, index, nb_bytes;
	WORD n;
		      		...
		      		if(cin == (USB_SINGLE_BYTE & 0x0F))
					{
			        ...
			        else // Non realtime message
		      		{
		         		...
		         		for(index=0; index < nb_bytes ; index++)
						{
		            		...
		            		MIDI_In_Add_Buffer(EP2FIFOBUF[index+frame+1], &MIDI_In_From_Usb);
							EA = 1;
							// DBG	Send_To_Midi_Out_Con(EP2FIFOBUF[index+frame+1]);
						}
