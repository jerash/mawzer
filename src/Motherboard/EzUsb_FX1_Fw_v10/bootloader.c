...
void Check_Bootloader(void)
{
		...
		if(must_program_z8)
		{
			...
			// set z8 dbg pin low
			FORCE_OCD_Z8_PORT_OUT &= ~FORCE_OCD_Z8_PIN;
...
void SendIdentityBootLoader(void)
{
  ...
  Serial_In_From_Z8.end_index   = JLABS_SYX_HEADER_SIZE + 4 + IDENTITY_LENGHT + 1;
...
void SendFwAcknoledge(unsigned char result)
{
  ...
  //Serial_In_From_Z8.start_index = 0;
  Serial_In_From_Z8.end_index   = JLABS_SYX_HEADER_SIZE + 1 + 1;
...
// ***************************************************************************
// Routine	: AssignJlabsSysexHeader()
// Input	: 
// Output	: None
// Process	: 
// ***************************************************************************
...
{
	// send Jlabs sysex header  
	Serial_In_From_Z8.buffer[0]   = MIDI_SOX;
	Serial_In_From_Z8.buffer[1]   = JLABS_SYX_ID0;
	Serial_In_From_Z8.buffer[2]   = JLABS_SYX_ID1;
	Serial_In_From_Z8.buffer[3]   = JLABS_SYX_ID2;
	Serial_In_From_Z8.buffer[4]   = JLABS_SYX_ID3;
	Serial_In_From_Z8.buffer[5]   = JLABS_SYX_ID4;
}
